...
WebUI.waitForElementVisible(findTestObject('WEB/Cart/lbl_ProductName'), 30)
...
qtyField.addProperty('xpath', ConditionType.EQUALS, '//input[@role="spinbutton"]')

TestObject btnPlus = new TestObject('btnPlus')

btnPlus.addProperty('xpath', ConditionType.EQUALS, '//span[contains(@class,"el-input-number__increase")]')
...
for (int i = 1; i <= 5; i++) {
    if (WebUI.verifyElementPresent(qtyField, 5, FailureHandling.OPTIONAL)) {
        qtyFound = true
...
// CLICK PLUS
//====================================================
WebUI.waitForElementClickable(btnPlus, 20)

WebUI.scrollToElement(btnPlus, 10)

WebUI.enhancedClick(btnPlus)
